...
namespace CookingHelper.LineDtoService
{
    public class RichMenuService
    {
        ...
        private readonly string uploadRichMenuImageUri =
            "https://api-data.line.me/v2/bot/richmenu/{0}/content";

        private readonly string setDefaultRichMenuUri =
            "https://api.line.me/v2/bot/user/all/richmenu/{0}";
        ...
        public async Task<string> ValidateRichMenu(RichMenuModel richMenu)
        {
            var jsonBody = new StringContent(
                _jsonProvider.Serialize(richMenu),
                Encoding.UTF8,
                "application/json"
            );
        ...
        public async Task<RichMenuListModel> GetRichMenuList()
        {
            ...
            var list = _jsonProvider.Deserialize<RichMenuListModel>(
        ...
        public async Task<string> UploadRichMenuImage(string richMenuId, IFormFile imageFile)
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                var request = new HttpRequestMessage(
                    HttpMethod.Post,
                    String.Format(uploadRichMenuImageUri, richMenuId)
                )
                {
                    Content = content
                };
        ...
        public async Task<string> SetDefaultRichMenu(string richMenuId)
        {
            var request = new HttpRequestMessage(
                HttpMethod.Post,
                String.Format(setDefaultRichMenuUri, richMenuId)
            );
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
                "Bearer",
                _LineBotChannelAccessToken
        ...
        public async Task<string> DeleteRichMenu(string richMenuId)
        {
            var request = new HttpRequestMessage(
                HttpMethod.Delete,
                String.Format(deleteRichMenuUri, richMenuId)
            );
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
                "Bearer",
                _LineBotChannelAccessToken
